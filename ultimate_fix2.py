...
for attempt in range(max_attempts):
    ...
    try:
        ...
        # 1. 모든 subject 참조 외래키 제약조건 찾기 및 삭제
        print("1. Dropping all foreign key constraints referencing subject...")
        cur.execute("""
            SELECT conname, conrelid::regclass, confrelid::regclass
            FROM pg_constraint
            WHERE confrelid = 'subject'::regclass AND contype = 'f';
        """)
        fk_constraints = cur.fetchall()
        print(f"   Found {len(fk_constraints)} FK constraints: {fk_constraints}")
        for conname, conrelid, confrelid in fk_constraints:
            try:
                cur.execute(f"ALTER TABLE {conrelid} DROP CONSTRAINT {conname};")
                print(f"   Dropped FK {conname} from {conrelid}")
            except Exception as e:
                print(f"   Error dropping {conname}: {e}")
        conn.commit()
        
        # 2. subject 테이블의 유니크 제약조건 삭제
        print("2. Dropping unique constraints on subject...")
        cur.execute("""
            SELECT conname FROM pg_constraint
            WHERE conrelid = 'subject'::regclass AND contype = 'u';
        """)
        unique_constraints = cur.fetchall()
        for (conname,) in unique_constraints:
            try:
                cur.execute(f"ALTER TABLE subject DROP CONSTRAINT {conname};")
                print(f"   Dropped unique constraint {conname}")
            except Exception as e:
                print(f"   Error dropping unique constraint {conname}: {e}")
        conn.commit()
        
        # 3. subject 테이블의 기본키 제약조건도 임시 삭제 (ID 변경을 위해)
        print("3. Dropping primary key constraint on subject...")
        cur.execute("""
            SELECT conname FROM pg_constraint
            WHERE conrelid = 'subject'::regclass AND contype = 'p';
        """)
        pk_constraints = cur.fetchall()
        for (conname,) in pk_constraints:
            try:
                cur.execute(f"ALTER TABLE subject DROP CONSTRAINT {conname};")
                print(f"   Dropped primary key {conname}")
            except Exception as e:
                print(f"   Error dropping PK {conname}: {e}")
        conn.commit()
        
        # 4. subject 테이블 비우기 (외래키 제약조건이 이미 삭제되었으므로)
        print("4. Clearing subject table...")
        cur.execute("DELETE FROM subject;")
        print(f"   Deleted all subjects")
        conn.commit()
        
        # 5. subject ID 1~6 삽입
        print("5. Inserting subjects with ID 1-6...")
        subjects_to_insert = [
        ...
        for subj_id, name, desc in subjects_to_insert:
            cur.execute("INSERT INTO subject (id, name, description) VALUES (%s, %s, %s);", 
                       (subj_id, name, desc))
            print(f"   Inserted subject ID {subj_id}: {name}")
        conn.commit()
        
        # 6. 기존 24문제 삭제 (question 텍스트로)
        print("6. Deleting old target problems...")
        target_questions = [
        ...
        # 7. 24문제 삽입 (subject_id 1~6 사용)
        print("7. Inserting 24 problems...")
        problems = [
            # Subject 1: 네트워크/웹
        ...
        if all_4 and len(rows) == 6 and count_1_6 == 24:
            ...
            # 9. 제약조건 복구 (선택 사항)
            print("9. Restoring constraints...")
            try:
                cur.execute("ALTER TABLE subject ADD CONSTRAINT subject_pkey PRIMARY KEY (id);")
                print("   Restored primary key")
            except Exception as e:
                print(f"   Could not restore PK: {e}")
            try:
                cur.execute("ALTER TABLE subject ADD CONSTRAINT subject_name_key UNIQUE (name);")
                print("   Restored unique constraint on name")
            except Exception as e:
                print(f"   Could not restore unique: {e}")
            # 외래키 복구는 생략 (필요시)
            conn.commit()
            cur.close()
            conn.close()
            ...
            print(f"\\n=== INCOMPLETE: count_1_6={count_1_6}, rows={len(rows)}, all_4={all_4} ===")
        cur.close()
        conn.close()
    except Exception as e:
