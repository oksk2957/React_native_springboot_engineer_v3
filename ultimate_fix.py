...
for attempt in range(max_attempts):
    ...
    try:
        ...
        print(f"   Deleted {total_deleted} old problems")
        conn.commit()
        
        # 3. Subject 테이블 정리 및 준비
        print("3. Preparing subjects...")
        
        # 목표: subject_id 1~6에 각각 해당 name을 가진 레코드 확보
        target_subjects = {
            1: '네트워크/웹',
            2: '데이터베이스', 
            3: 'Java/Spring',
        ...
        # 기존 subject 중 name이 중복되지 않도록 정리
        # 먼저 각 target name이 이미 어떤 ID에 있는지 확인
        name_to_id = {}
        cur.execute("SELECT id, name FROM subject;")
        for id, name in cur.fetchall():
            name_to_id[name] = id
        
        print(f"   Current name->id mapping: {name_to_id}")
        
        # 각 target에 대해 처리
        for target_id, target_name in target_subjects.items():
            # 이미 해당 name을 가진 레코드가 있는지 확인
            if target_name in name_to_id:
                existing_id = name_to_id[target_name]
                print(f"   Name '{target_name}' already exists at ID {existing_id}")
                # target_id와 다르면, 문제의 subject_id를 기존 ID로 매핑해야 함
                # 하지만 우리는 무조건 subject_id 1~6을 사용하길 원하므로,
                # 기존 레코드의 ID를 target_id로 변경하거나, target_id의 레코드를 가져와야 함
                # ID를 직접 변경하는 것은 위험하므로, target_id에 해당 name으로 업데이트 시도
                # 단, 다른 레코드가 이미 그 name을 가지고 있으면 충돌
                # 따라서, target_id에 있는 레코드의 name을 target_name으로 바꾸고,
                # 기존의 중복 name 레코드는 삭제하거나 이름 변경
                
                # 간단히: target_id에 해당 name이 없으면, target_id의 name을 target_name으로 업데이트
                cur.execute("SELECT name FROM subject WHERE id = %s;", (target_id,))
                row = cur.fetchone()
                if row:
                    current_name = row[0]
                    if current_name != target_name:
                        # 업데이트 시도
                        try:
                            cur.execute("UPDATE subject SET name = %s WHERE id = %s;", (target_name, target_id))
                            print(f"   Updated subject ID {target_id} name to '{target_name}'")
                        except psycopg2.errors.UniqueViolation:
                            conn.rollback()
                            cur = conn.cursor()
                            # 중복 발생: target_id의 현재 name을 다른 이름으로 바꾸고 다시 시도
                            temp_name = f"temp_{target_id}_{int(time.time())}"
                            cur.execute("UPDATE subject SET name = %s WHERE id = %s;", (temp_name, target_id))
                            conn.commit()
                            cur.execute("UPDATE subject SET name = %s WHERE id = %s;", (target_name, target_id))
                            conn.commit()
                            print(f"   Resolved conflict, updated subject ID {target_id} to '{target_name}'")
                else:
                    # target_id에 레코드가 없으면 삽입
                    try:
                        cur.execute("INSERT INTO subject (id, name) VALUES (%s, %s);", (target_id, target_name))
                        print(f"   Inserted subject ID {target_id}: '{target_name}'")
                    except psycopg2.errors.UniqueViolation:
                        conn.rollback()
                        cur = conn.cursor()
                        # 이미 다른 ID에 name이 있으므로, 그 ID를 사용하도록 문제 삽입 시 조정
                        print(f"   Name '{target_name}' exists elsewhere, will adjust insertion")
            
            else:
                # name이 전혀 없으면, target_id에 삽입 (없으면) 또는 업데이트
                cur.execute("SELECT id FROM subject WHERE id = %s;", (target_id,))
                if cur.fetchone():
                    try:
                        cur.execute("UPDATE subject SET name = %s WHERE id = %s;", (target_name, target_id))
                        print(f"   Updated subject ID {target_id} to '{target_name}'")
                    except psycopg2.errors.UniqueViolation:
                        conn.rollback()
                        cur = conn.cursor()
                        print(f"   Conflict updating ID {target_id}, will handle later")
                else:
                    try:
                        cur.execute("INSERT INTO subject (id, name) VALUES (%s, %s);", (target_id, target_name))
                        print(f"   Inserted subject ID {target_id}: '{target_name}'")
                        ...
                        conn.rollback()
                        cur = conn.cursor()
                        print(f"   Error inserting subject ID {target_id}: {e}")
        
        conn.commit()
        
        # 4. 최종 확인: subject_id 1~6의 name 확인
        print("4. Verifying subjects 1-6...")
        cur.execute("SELECT id, name FROM subject WHERE id BETWEEN 1 AND 6 ORDER BY id;")
        rows = cur.fetchall()
        subject_id_map = {}
        for id, name in rows:
            print(f"   ID {id}: {name}")
            subject_id_map[name] = id
        
        # 5. 24문제 삽입 - subject_id 1~6 사용
        print("5. Inserting 24 problems with subject_id 1-6...")
        
        # 각 문제에 올바른 subject_id 배정
        # subject_id 1: 네트워크/웹 (4문제)
        # subject_id 2: 데이터베이스 (4문제)
        # subject_id 3: Java/Spring (4문제)
        # subject_id 4: 운영체제 (4문제)
        # subject_id 5: 웹 보안 (4문제)
        # subject_id 6: 머신러닝 (4문제)
        ...
        for p in problems:
            try:
                ...
                # 개별 문제 삽입 실패 시 롤백하고 계속
                conn.rollback()
                cur = conn.cursor()
        
        conn.commit()
        print(f"   Insertion attempted for {len(problems)} problems")
        
        # 6. 검증
        print("6. Verification...")
        cur.execute("SELECT COUNT(*) FROM problem WHERE subject_id BETWEEN 1 AND 6;")
        count_1_6 = cur.fetchone()[0]
        print(f"   Problems with subject_id 1-6: {count_1_6}")
        ...
        if all_4 and len(rows) == 6 and count_1_6 == 24:
            print("\\n=== SUCCESS: Exactly 24 problems with subject_id 1-6, 4 each ===")
            cur.close()
            conn.close()
            break
        else:
