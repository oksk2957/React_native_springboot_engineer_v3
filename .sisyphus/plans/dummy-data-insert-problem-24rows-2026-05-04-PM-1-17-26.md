# [PostgreSQL] problem 테이블 더미 데이터 INSERT 쿼리 (24개)

## 📋 DDL 제약조건 (MCP 검증 완료)

### Problem 테이블 제약조건
- **id**: bigserial (자동 증가, INSERT 시 생략 가능)
- **subject_id**: int8 NOT NULL, FOREIGN KEY → subject(id)
- **type**: text NOT NULL, CHECK (type = 'SUBJECTIVE' OR 'OBJECTIVE')
- **difficulty**: int4 NULL, CHECK (difficulty >= 1 AND difficulty <= 5)
- **question**: text NOT NULL
- **answer**: text NOT NULL
- **explanation**: text NULL
- **option1~5**: text NULL (OBJECTIVE는 필수, SUBJECTIVE는 NULL 가능)
- **is_ai_generated**: bool DEFAULT false

### MCP 검증 결과
- **Library**: /websites/postgresql (Score: 77.8)
- **검증 1**: bigserial 컬럼은 INSERT 시 값 생략 시 자동 증가
...
('네트워크', 'TCP/IP, HTTP, DNS 등'),
('Java 프로그래밍', 'OOP, Spring, Java 문법'),
('운영체제', '프로세스, 메모리, 스케줄링'),
...
VALUES (1, 'test', 'test', 'INVALID_TYPE', 1);
-- ERROR: new row for relation "problem" violates check constraint "problem_type_check"

-- difficulty 위반 (실패해야 함)
INSERT INTO problem (subject_id, question, answer, type, difficulty) 
VALUES (1, 'test', 'test', 'OBJECTIVE', 6);
-- ERROR: new row for relation "problem" violates check constraint "problem_difficulty_check"
```
...
### Step 1: subject 데이터 확인/삽입
```sql
-- subject 테이블 확인
SELECT * FROM subject ORDER BY id LIMIT 6;

-- 없으면 위의 subject INSERT 쿼리 실행
```

### Step 2: problem 더미 데이터 삽입
```sql
-- 위의 6개 INSERT 쿼리 실행 (또는 하나로 합쳐서 실행)
```
...
WHERE CAST(p.type AS TEXT) = 'OBJECTIVE'
