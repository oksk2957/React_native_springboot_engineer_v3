...
for attempt in range(max_attempts):
    ...
    try:
        ...
        # 1. 기존 24문제 삭제
        print("1. Deleting target 24 problems...")
        target_questions = [
        ...
        print(f"   Deleted {total_deleted} problems")
        conn.commit()
        
        # 2. subject 테이블 정리: name 유니크 제약조건 임시 해제
        print("2. Modifying subject table constraints...")
        ...
        for (conname,) in constraints:
            try:
                ...
                print(f"   Note: Could not drop {conname}: {e}")
        
        conn.commit()
        
        # 3. subject 테이블 비우기 (외래키 참조 문제 해결하기 위해 problem부터 정리)
        print("3. Clearing subject table...")
        
        # problem 테이블의 외래키 임시 해제
        try:
            cur.execute("ALTER TABLE problem DROP CONSTRAINT problem_subject_id_fkey;")
            print("   Dropped foreign key constraint from problem")
        except Exception as e:
        ...
        # 4. subject ID 1~6 삽입
        print("4. Inserting subjects with ID 1-6...")
        subjects_to_insert = [
        ...
        # 5. 외래키 다시 추가
        print("5. Re-adding foreign key constraint...")
        try:
            cur.execute("ALTER TABLE problem ADD CONSTRAINT problem_subject_id_fkey FOREIGN KEY (subject_id) REFERENCES subject(id) ON DELETE CASCADE;")
            print("   Re-added foreign key constraint")
        except Exception as e:
            print(f"   Note: Could not re-add FK: {e}")
        
        # name 유니크 제약조건 다시 추가
        try:
            cur.execute("ALTER TABLE subject ADD CONSTRAINT subject_name_key UNIQUE (name);")
            print("   Re-added unique constraint on subject.name")
        except Exception as e:
