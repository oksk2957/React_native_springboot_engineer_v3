...
INSERT INTO problem (subject_id, question, answer, explanation, type, difficulty, option1, option2, option3, option4, option5, is_ai_generated) VALUES
(3, 'Java에서 객체 지향의 4대 원칙은?', '캡슐화, 상속, 추상화, 다형성', 'OOP의 핵심 개념', 'OBJECTIVE', 2, '캡슐화, 상독, 추상화, 다형성', '입력, 처리, 출력, 저장', '변수, 함수, 클래스, 객체', '컴파일, 실행, 디버깅, 배포', NULL, false),
...
for attempt in range(max_retries):
    try:
        ...
        if attempt < max_retries - 1:
            print("Retrying after modification...")
            # Try to fix: maybe remove the ON CONFLICT DO NOTHING if subject table doesn't have id as primary key?
            # Or maybe the subject table doesn't exist? Let's check.
            # We'll modify the SQL accordingly.
