...
for attempt in range(max_attempts):
    ...
    try:
        ...
        # 1. subject_id 1~6의 모든 문제 삭제 (무조건)
        print("1. Deleting ALL problems with subject_id 1-6...")
        cur.execute("DELETE FROM problem WHERE subject_id BETWEEN 1 AND 6;")
        deleted = cur.rowcount
        print(f"   Deleted {deleted} problems")
        conn.commit()
        
        # 2. subject 테이블 정리: ID 1~6만 남기고 나머지 삭제
        print("2. Ensuring only subjects 1-6 exist...")
        cur.execute("DELETE FROM subject WHERE id NOT BETWEEN 1 AND 6;")
        print(f"   Deleted extra subjects")
        conn.commit()
        
        # 3. subject ID 1~6 이름 업데이트 (계획대로)
        print("3. Updating subject names for ID 1-6...")
        subject_map = {
        ...
        for sid, (name, desc) in subject_map.items():
            cur.execute("UPDATE subject SET name = %s, description = %s WHERE id = %s;", (name, desc, sid))
            if cur.rowcount == 0:
                cur.execute("INSERT INTO subject (id, name, description) VALUES (%s, %s, %s);", (sid, name, desc))
                print(f"   Inserted subject ID {sid}: {name}")
            else:
                print(f"   Updated subject ID {sid}: {name}")
        conn.commit()
        
        # 4. 24문제 삽입 (과목당 4개)
        print("4. Inserting 24 problems (4 per subject)...")
        problems = [
        ...
        if all_4 and len(rows) == 6 and count_1_6 == 24:
            ...
            print(f"\\n=== INCOMPLETE: count={count_1_6}, subjects={len(rows)}, all_4={all_4} ===")
        cur.close()
        conn.close()
