...
CREATE TABLE problem (
	...
	CONSTRAINT problem_type_check CHECK ((type = 'OBJECTIVE'::text)),
	CONSTRAINT problem_subject_id_fkey FOREIGN KEY (subject_id) REFERENCES subject(id) ON DELETE CASCADE
);


-- public.subjective_problems definition (주관식 전용; 기존 problem 의 SUBJECTIVE 행 마이그레이션 대상)
...
-- public.programming_language_problems definition (단일 테이블 + 언어 구분)
...
CREATE TABLE study_session (
	...
	CONSTRAINT study_session_problem_branch_check CHECK ((problem_branch IS NULL OR (problem_branch = ANY (ARRAY['OBJECTIVE'::text, 'SUBJECTIVE'::text, 'PROGRAMMING_LANGUAGE'::text])))),
...
CREATE TABLE user_answer (
	...
	CONSTRAINT user_answer_user_branch_ref_key UNIQUE (user_id, problem_type, reference_id),
	CONSTRAINT user_answer_problem_type_check CHECK ((problem_type = ANY (ARRAY['OBJECTIVE'::text, 'SUBJECTIVE'::text, 'PROGRAMMING_LANGUAGE'::text]))),
...
-- public.user_unified_statistics definition (줄기별·선택적 세부 reference 단위 롤업; reference_id NULL 이면 줄기 전체)

CREATE TABLE user_unified_statistics (
	id bigserial NOT NULL,
	user_id int8 NOT NULL,
	problem_type text NOT NULL,
...
CREATE UNIQUE INDEX ux_user_unified_statistics_branch ON user_unified_statistics (user_id, problem_type) WHERE reference_id IS NULL;
CREATE UNIQUE INDEX ux_user_unified_statistics_item ON user_unified_statistics (user_id, problem_type, reference_id) WHERE reference_id IS NOT NULL;
...
CREATE TABLE wrong_answer_bookmark (
	...
	CONSTRAINT wrong_answer_bookmark_user_branch_ref_key UNIQUE (user_id, problem_type, reference_id),
	CONSTRAINT wrong_answer_bookmark_problem_type_check CHECK ((problem_type = ANY (ARRAY['OBJECTIVE'::text, 'SUBJECTIVE'::text, 'PROGRAMMING_LANGUAGE'::text]))),
